...
export default async function CalendarioPage({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Celular: agenda em lista (só dias com postagem) */}
      <div className="space-y-2 sm:hidden">
        {posts.length === 0 ? (
          <p className="text-sm text-muted-foreground">Nenhuma postagem programada neste mês.</p>
        ) : (
          [...porDia.keys()].sort((a, b) => a - b).map((dia) => (
            <div key={dia} className="rounded-lg border border-border">
              <p className={`border-b border-border px-3 py-1.5 text-sm font-semibold ${dia === hojeNoMes ? "text-primary" : ""}`}>
                {dia} de {MESES[mes0]}{dia === hojeNoMes ? " · hoje" : ""}
              </p>
