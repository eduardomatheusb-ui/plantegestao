...
/** Monta um e-mail simples com a cara do TREM/Plante. */
export function layoutEmail(opts: {
  ...
  const { titulo, corpo, linkUrl, linkTexto = "Abrir no TREM", rodape } = opts;
  const botao = linkUrl
  ...
  <div style="max-width:520px;margin:0 auto;padding:24px">
    <div style="background:#050505;color:#F7FF19;font-weight:800;font-size:22px;letter-spacing:1px;padding:16px 20px;border-radius:12px 12px 0 0">TREM</div>
    ...
    <p style="font-size:11px;color:#9ca3af;text-align:center;margin-top:16px">
      ${rodape ?? "Você recebeu este e-mail porque participa do TREM — sistema interno da Plante Comunicação."}
    </p>
  </div></body></html>`;
}
