...
import { salvarProducao, type ProducaoFormState } from "@/lib/producao/actions";
...
export type ProducaoInicial = Partial<Record<
  "titulo" | "clienteId" | "fornecedorId" | "projetoId" | "versao" | "dataEntrega" | "vencimento" | "formaPagamento" | "comissaoPct" | "observacao",
  string
>>;
...
export function ProducaoForm({
  ...
  id: string | null;
  inicial?: ProducaoInicial;
  clientes: Opt[];
  fornecedores: Opt[];
  projetos: ProjetoOpt[];
  cancelHref: string;
}) {
  const action = salvarProducao.bind(null, id);
  const [state, formAction] = useActionState<ProducaoFormState, FormData>(action, {});
    ...
    <form action={formAction} className="space-y-6" noValidate>
      ...
      <div className="grid grid-cols-1 gap-x-4 gap-y-5 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input id="dataEntrega" name="dataEntrega" type="date" defaultValue={inicial.dataEntrega ?? ""} />
        </div>
        <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <Input id="comissaoPct" name="comissaoPct" type="number" step="0.01" min="0" defaultValue={inicial.comissaoPct ?? "0"} />
        </div>
        <div className="space-y-2 sm:col-span-2">
